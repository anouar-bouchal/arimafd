...
class diff_integ:
    ...
    def fit_transform(self, data, return_diff=True):
        ...
        data = np.array(data)
        if (len(data) - sum(self.seasons) <= sum(self.seasons)) or (
            len(self.seasons) < 1
        ):
    ...
    def inverse_fit_transform0(self):
        ...
        """
        self.Sum_insstead_Minuend[len(self.seasons)] = self.Difference[
            len(self.seasons) - 1
        ]
        j = 0
        for i in range(len(self.seasons) - 1, -1, -1):
            self.Sum_insstead_Minuend[i] = (
                self.Sum_insstead_Minuend[i + 1]
                + self.Subtrahend[i][sum(self.seasons[::-1][:j]) :]
            )
            j += 1
    ...
    def inverse_transform(self, new_value):
        ...
        for i in range(len(self.seasons) - 1, -1, -1):
            self.Sum_insstead_Minuend[i] = (
                self.Sum_insstead_Minuend[i + 1] + self.additional_term[i]
            )
