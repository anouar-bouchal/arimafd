...
class find_best_model:
    def __init__(
        self, data, max_ar, max_ma, verbose=True, find_online_ar=True, criterion="mae"
    ):
        """
        ...
        ar_aic = (
            table_aic[table_aic == np.nanmin(table_aic.values)]
        ...
        ar_bic = (
            table_bic[table_bic == np.nanmin(table_bic.values)]
            .dropna(axis=1, how="all")
            .dropna(how="all")
            .index.item()
        )
        ...
        ar_mae = (
            table_mae[table_mae == np.nanmin(table_mae.values)]
            .dropna(axis=1, how="all")
            .dropna(how="all")
            .index.item()
        )
