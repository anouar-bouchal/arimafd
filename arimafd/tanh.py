...
class online_tanh:
    def __init__(
        self, order=4, lrate=0.001, random_state=42, soft_grad=False, project=True
    ):
        """
        ...
        if soft_grad:
            ...
            def fun_w(i):
                return 1 / np.sqrt(
                    i + 1
                )  # намерено опустил  член -order, из-за небольшой погрешности допущения
    ...
    def fit(self, data, init_w=None):
        ...
        self.w = (
            np.random.rand(self.order + 1) * 0.01 if init_w is None else init_w.copy()
        )
    ...
    def predict(self, point_get=None, predict_size=1, return_predict=True):
        ...
        if point_get is not None:
            ...
            self.diff[:-1] = (
                np.tanh(self.pred[-1] - self.data[-1]) * self.data[-self.order - 1 : -1]
            )
...
class Anomaly_detection:
    ...
    def proc_tensor(self, window=100, No_metric=1, window_insensitivity=100):
        ...
        df = pd.DataFrame(
            tensor.reshape(len(tensor), -1), index=self.indices[-len(tensor) :]
        )
        # mean and min in end is the euthrestic
        #         if len(self.data.iloc[:,i].unique())/len(self.data.iloc[:,i]) > 0.99:
        if No_metric == 1:
            metric = (
                df.rolling(window).max().abs() / df.rolling(window).std().abs()
            ).mean(axis=1)
        #             print('Metric1')
        elif No_metric == 2:
            ...
            metric = (
                ...
                .apply(
                    lambda x: int(
                        np.abs(
                            (x.values[-1] - np.mean(x)) / np.std(x) > koef * np.std(x)
                        )
                    )
                )
                .sum(axis=1)
